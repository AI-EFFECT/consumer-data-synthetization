...
Edit the constants at the top of `train.py` — everything the script needs to run
lives here, so `python train.py` with no arguments works out of the box:

```python
DEVICE_IDS = ["100", "CLITCE000134"]
...
One model is trained per `device_id`/`parameter` pair. `Smart_Meters_IDs`
above `DEVICE_IDS` is the full known device list, kept for reference — copy
the ids you actually want into `DEVICE_IDS`. Any of these can also be
overridden per-run with the matching CLI flag (`--device-ids`,
`--parameters`, `--start-date`, ...) without editing the file.
...
naming families are not a reliable guide (the `CLCECE` block is mixed), so the
